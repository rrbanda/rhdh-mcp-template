"""${{ values.name }} -- FastMCP server wrapping a REST API as MCP tools."""
...
mcp = FastMCP("${{ values.name }}")

API_BASE_URL = os.environ.get("API_BASE_URL", "${{ values.apiBaseUrl }}")
...
if __name__ == "__main__":
    mcp.run(transport="streamable-http", host="0.0.0.0", port=${{ values.port }})
